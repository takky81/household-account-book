...
--
-- カテゴリは共通マスタになったので、ルールが持つ共有範囲はルール自身の列で決まる。
-- 「参照できないカテゴリのルールは動かさない」は「参照できない共有範囲」に読み替わる。
...
-- カテゴリは全員共通。共有範囲はルールの側で決める
insert into public.categories (kind, name)
select 'expense', name
  from (values ('pg-家賃'), ('pg-定期-停止'), ('pg-定期-終了'), ('pg-定期-未来'),
               ('pg-定期-手入力'), ('pg-定期-アーカイブ'), ('pg-定期-電気'), ('pg-定期-水道'),
               ('pg-個人-サブスク')) as n(name);

create temp table cat as
select n.name, (select c.id from public.categories c where c.name = n.name) as id
  from (values ('pg-家賃'), ('pg-定期-停止'), ('pg-定期-終了'), ('pg-定期-未来'),
               ('pg-定期-手入力'), ('pg-定期-アーカイブ'), ('pg-定期-電気'), ('pg-定期-水道'),
               ('pg-個人-サブスク')) as n(name);
...
select throws_ok(
  format($$ select public.upsert_recurring_rule(%L, 0, 1, %L, p_share_group_id => %L) $$,
         (select id from cat where name = 'pg-家賃'), (select m0 from cal), (select gid from fx)),
  'P0001', null, '定期登録ルールの管理 列5 0円のルールは作れない'
);

select throws_ok(
  format($$ select public.upsert_recurring_rule(%L, 1000, 32, %L, p_share_group_id => %L) $$,
         (select id from cat where name = 'pg-家賃'), (select m0 from cal), (select gid from fx)),
...
select throws_ok(
  format($$ select public.upsert_recurring_rule(%L, 1000, 1, %L, null, '', %L,
             p_share_group_id => %L) $$,
         (select id from cat where name = 'pg-家賃'), (select m0 from cal),
         (select m1 from cal), (select gid from fx)),
...
select throws_ok(
  format($$ select public.upsert_recurring_rule(%L, 1000, 1, %L, %L,
         ...
         (select id from cat where name = 'pg-個人-サブスク'), (select m0 from cal),
         '22222222-2222-2222-2222-222222222222'),
  'P0001', null, '定期登録ルールの管理 列8 個人の共有範囲の支払者は本人だけ'
);
...
select throws_ok(
  format($$ select public.upsert_recurring_rule(%L, 1000, 1, %L,
         ...
         (select id from cat where name = 'pg-家賃'), (select m0 from cal)),
  'P0001', null, '定期登録ルールの管理 列7 参照できない共有範囲には作れない'
);

-- 正常系。自動按分のルールは雛形を持たない
create temp table rules as
select 'pg-家賃' as name, public.upsert_recurring_rule(
  (select id from cat where name = 'pg-家賃'), 120000, 1, (select m2 from cal),
...
insert into rules
select 'pg-定期-手入力', public.upsert_recurring_rule(
  (select id from cat where name = 'pg-定期-手入力'), 3000, 1, (select m0 from cal),
...
select is(
  (select array_agg(s.amount order by s.amount desc)
    ...
    where s.rule_id = rules.id and rules.name = 'pg-定期-手入力'),
  array[2000, 1000],
...
-- 生成されないルールたち
insert into rules
select 'pg-定期-停止', public.upsert_recurring_rule(
  (select id from cat where name = 'pg-定期-停止'), 500, 1, (select m2 from cal),
  null, '', null, true, p_share_group_id => (select gid from fx));
insert into rules
select 'pg-定期-終了', public.upsert_recurring_rule(
  (select id from cat where name = 'pg-定期-終了'), 500, 1, (select m2 from cal),
  null, '', (select m2 from cal), p_share_group_id => (select gid from fx));
insert into rules
select 'pg-定期-未来', public.upsert_recurring_rule(
  (select id from cat where name = 'pg-定期-未来'), 500, 1, (select mn from cal),
  p_share_group_id => (select gid from fx));
insert into rules
select 'pg-定期-アーカイブ', public.upsert_recurring_rule(
  (select id from cat where name = 'pg-定期-アーカイブ'), 500, 1, (select m2 from cal),
  p_share_group_id => (select gid from fx));

update public.categories set is_archived = true
 where id = (select id from cat where name = 'pg-定期-アーカイブ');
...
select is(
  (select count(*)::int from public.transactions t, cat
    where t.category_id = cat.id and cat.name = 'pg-家賃'),
...
select is(
  (select count(*)::int from public.transactions t, cat
    where t.category_id = cat.id and cat.name = 'pg-定期-未来'),
...
select is(
  (select count(*)::int from public.transactions t, cat
    where t.category_id = cat.id and cat.name = 'pg-定期-停止'),
  0, '定期登録の生成 列5 一時停止のルールは作らない'
);

select is(
  (select count(*)::int from public.transactions t, cat
    where t.category_id = cat.id and cat.name = 'pg-定期-終了'),
  1, '定期登録の生成 列6 終了月を過ぎた対象月は作らない'
);

select is(
  (select count(*)::int from public.transactions t, cat
    where t.category_id = cat.id and cat.name = 'pg-定期-アーカイブ'),
...
select is(
  (select array_agg(s.amount order by s.amount)
     ...
     join public.transactions t on t.id = s.transaction_id
     join cat on cat.id = t.category_id
    where cat.name = 'pg-家賃' and t.occurred_on = (select m0 from cal)),
  array[60000, 60000],
...
delete from public.transactions t
 where t.id = (select p.transaction_id from public.recurring_postings p, rules
                where p.rule_id = rules.id and rules.name = 'pg-家賃'
...
-- 参照できない共有範囲のルールは他人の起動で動かない。
-- カテゴリは共通なので、隠すのは共有範囲の方になった
create temp table hana_rule as
select public.upsert_recurring_rule(
  (select id from cat where name = 'pg-個人-サブスク'), 800, 1, (select m0 from cal),
...
select is(
  ...
  0, '定期登録の生成 列15 参照できない共有範囲のルールは動かさない'
);

-- 支払者が抜けても、他のルールは止まらない
insert into rules
select 'pg-定期-電気', public.upsert_recurring_rule(
  (select id from cat where name = 'pg-定期-電気'), 7000, 1, (select m0 from cal),
  '22222222-2222-2222-2222-222222222222', '電気',
  p_share_group_id => (select gid from fx));
insert into rules
select 'pg-定期-水道', public.upsert_recurring_rule(
  (select id from cat where name = 'pg-定期-水道'), 3000, 1, (select m0 from cal),
  null, '水道', p_share_group_id => (select gid from fx));
...
select is(
  (select count(*)::int from public.transactions t, cat
    where t.category_id = cat.id and cat.name = 'pg-定期-電気'),
  0, '定期登録の生成 列10 支払者が抜けたルールは登録できない'
);

select is(
  (select count(*)::int from public.transactions t, cat
    where t.category_id = cat.id and cat.name = 'pg-定期-水道'),
...
select throws_ok(
  format($$ insert into public.recurring_postings (rule_id, month) values (%L, %L) $$,
         (select id from rules where name = 'pg-家賃'), (select m0 from cal)),
  '42501', null, '定期登録の生成 列11 生成の記録は直接 INSERT できない'
);

-- ルールを消しても、作った取引は残る
delete from public.recurring_rules r where r.id = (select id from rules where name = 'pg-家賃');
